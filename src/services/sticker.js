...
export async function processStaticSticker(inputPath, metadata) {
    return new Promise((resolve, reject) => {
        ...
        const cmd = `ffmpeg -y -i "${inputPath}" -vf "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=black@0" -vframes 1 -q:v 80 -f webp "${outFile}"`;
